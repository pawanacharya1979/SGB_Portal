...
class UploadsController extends Controller
{
    ...
    public function upload_files() {
		...
		if(Module::hasAccess("Uploads", "create")) {
			...
			if(Input::hasFile('file')) {
				...
				$cfilename=str_replace(' ','-',$filename);
				
				$date_append = date("Y-m-d-His-");
				$upload_success = Input::file('file')->move($folder, $date_append.$cfilename);
				...
				if( $upload_success ) {
					...
					$upload = Upload::create([
						"name" => $filename,
						"path" => $folder.DIRECTORY_SEPARATOR.$date_append.$cfilename,
						"extension" => pathinfo($cfilename, PATHINFO_EXTENSION),
						"caption" => "",
						"hash" => "",
						"public" => $public,
    ...
    public function uploaded_files()
    {
		if(Module::hasAccess("Uploads", "view")) {
			...
			if(Entrust::hasRole('SUPER_ADMIN')) {
				//$uploads = Upload::all();
				$uploads = Upload::orderBy('name', 'ASC')->get();
			} else {
			...
			foreach ($uploads as $upload) {
				...
				$u->path = $this->bs_url.str_replace($this->temp_path,'',$upload->path);
    ...
    public function delete_file()
    {
        if(Module::hasAccess("Uploads", "delete")) {
			...
			if(isset($upload->id)) {
				if($upload->user_id == Auth::user()->id || Entrust::hasRole('SUPER_ADMIN')) {
					/*$cds = Course_Detail::where(function ($query) {
						    $query->where($upload->id, '=','courseprogram')
						          ->orWhere($upload->id, '=', 'traveldetails');
						})->get();
					foreach ($cds as $cd) {
						$course_detail = Course_Detail::find($cd->id);
						 if($cd->courseprogram == $upload->id){
							$course_detail->update(['courseprogram' => '0']);
					    }
					    if($cd->traveldetails == $upload->id){
