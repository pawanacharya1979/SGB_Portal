...
class StudentController extends Controller
{
	...
	public function store(Request $request)
	{
		if(Module::hasAccess("Users", "create")) {
			...
			return redirect()->route(config('laraadmin.adminRoute') . '.student.index');
		...
		for($i=0; $i < count($data->data); $i++) {
			for ($j=0; $j < count($this->listing_cols); $j++) { 
				...
				if($col == $this->view_col) {
					$data->data[$i][$j] = '<a href="'.url(config('laraadmin.adminRoute') . '/student/'.$data->data[$i][0]).'/edit">'.$data->data[$i][$j].'</a>';
				}
				// else if($col == "author") {
				//    $data->data[$i][$j];
				// }
			...
			if($this->show_action) {
				$output = '';
				if(Module::hasAccess("Users", "edit")) {
					$output .= '<a href="'.url(config('laraadmin.adminRoute') . '/student/'.$data->data[$i][0].'/edit').'" class="btn btn-warning btn-xs" style="display:inline;padding:2px 5px 3px 5px;"><i class="fa fa-edit"></i></a>';
				}
				
				if(Module::hasAccess("Users", "delete")) {
					$output .= Form::open(['route' => [config('laraadmin.adminRoute') . '.student.destroy', $data->data[$i][0]], 'method' => 'delete', 'style'=>'display:inline','id'=>'deleteuser'.$data->data[$i][0]]);
