...
class EmployeeController extends Controller
{
	...
	public $listing_cols = ['id', 'email', 'password', 'firstname', 'lastname', 'notes', 'userappform'];
	...
	public function store(Request $request)
	{
		if(Module::hasAccess("Users", "create")) {
			...
			return redirect()->route(config('laraadmin.adminRoute') . '.employee.index');
	...
	{
		$values = DB::table('users')->select($this->listing_cols)->whereNull('deleted_at')->where('role','1');
		...
		for($i=0; $i < count($data->data); $i++) {
			for ($j=0; $j < count($this->listing_cols); $j++) { 
				...
				if($col == $this->view_col) {
					$data->data[$i][$j] = '<a href="'.url(config('laraadmin.adminRoute') . '/employee/'.$data->data[$i][0]).'">'.$data->data[$i][$j].'</a>';
				}
				// else if($col == "author") {
				//    $data->data[$i][$j];
				// }
			...
			if($this->show_action) {
				$output = '';
				if(Module::hasAccess("Users", "edit")) {
					$output .= '<a href="'.url(config('laraadmin.adminRoute') . '/employee/'.$data->data[$i][0].'/edit').'" class="btn btn-warning btn-xs" style="display:inline;padding:2px 5px 3px 5px;"><i class="fa fa-edit"></i></a>';
				}
				
				if(Module::hasAccess("Users", "delete")) {
					$output .= Form::open(['route' => [config('laraadmin.adminRoute') . '.employee.destroy', $data->data[$i][0]], 'method' => 'delete', 'style'=>'display:inline','id'=>'deleteuser'.$data->data[$i][0]]);
					$output .= ' <button class="btn btn-danger btn-xs" type="submit" onclick="deleteRow(deleteuser'.$data->data[$i][0].')"><i class="fa fa-times"></i></button>';
					$output .= Form::close();
				}
