...
<form action="{{route(config('laraadmin.adminRoute').'.la_configs.store')}}" method="POST">
	...
	<div class="box box-warning">
		...
		<div class="box-body">
			...
			<div class="form-group">
				<label>Default Email Address(from email)</label>
				<input type="text" class="form-control" placeholder="To send emails to others via SMTP" maxlength="100" name="default_email" value="{{$configs->default_email}}" required>
			</div>
			<div class="form-group">
				<label>Default Email Body</label>
				<textarea class="form-control" name="default_emailbody">{{$configs->default_emailbody}}</textarea>
			</div>
		</div><!-- /.box-body -->
