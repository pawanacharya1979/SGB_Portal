...
$(document).ready(function() {
    ...
    function getLI(upload) {
        ...
        return '<li><a class="fm_file_sel" data-toggle="tooltip" data-placement="top" title="'+upload.name+'" upload=\''+JSON.stringify(upload)+'\'>'+image+ '<span class="file_name">'+upload.name +'</span></a></li>';
    }
    ...
    fm_dropzone = new Dropzone("#fm_dropzone", {
        maxFilesize: 20,
        acceptedFiles: "image/*,application/pdf, application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.wordprocessingml.document,application/docx,.doc,text/csv,text/html,text/plain,text/tab-separated-values,application/xls,application/excel,application/vnd.ms-excel,application/vnd.ms-excel; charset=binary,application/msexcel,application/x-excel,application/x-msexcel,application/x-ms-excel,application/x-dos_ms_excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        init: function() {
    ...
    $("body").on("click", ".fm_file_sel", function() {
        ...
        if(type == "image") {
            ...
            $hinput.next("a").next(".uploaded_image").append('<span class="file_name">'+upload.name+'</span>');
        } else if(type == "file") {
            $hinput = $("input[name="+$("#image_selecter_origin").val()+"]");
            $hinput.val(upload.id);
            ...
            $hinput.next("a").next(".uploaded_file").append('<span class="file_name">'+upload.name+'</span>');
            ...
            $hinput.next("div.uploaded_files").append("<a onclick='removeRecord(this);return false;' class='uploaded_file2' upload_id='"+upload.id+"' target='_blank' href='"+upload.path+"'>"+fileImage+"<i title='Remove File' class='fa fa-times'></i><span class='file_name'>"+upload.name+"</span></a>");
        }
        $("#fm").modal('hide');
    });
